...
class PagSeguroErrorHandlerTest {
    ...
    fun `400 with valid JSON throws ValidationError`() {
        ...
        val ex = assertThrows<PagSeguroException.ValidationError> { handler.handle(response) }
    ...
    fun `500 throws ServerError with status code`() {
        ...
        val ex = assertThrows<PagSeguroException.ServerError> { handler.handle(response) }
